...
function zc_createClient(documentId, processor) {
    ...
    function requestStatusHint(status) {
        if (status >= 300) {
            if (status < 400) {
                alert(`Received unexpected redirection message ${status}!`);
            }
            else if (status < 500) {
                ...
                if (status === 503) {
                    alert('Zotero is serving another program, restart it if this is not the case.');
                }
                else {
    ...
    function transact(command) {
        ...
        try {
            ...
            if (error.name === 'NetworkError') {
                alert('Network error occurred, is Zotero running?' + guide);
            }
            else {
                alert(`Error occurred ${error.name}, please click dev tool, navigate to console and report the issue.` + guide);
            }
        }
