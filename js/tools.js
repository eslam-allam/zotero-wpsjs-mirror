...
// 节点包装器（适配 from-xml 数据结构）
class FromXmlNodeWrapper {
    constructor(nodeObj) {
        this.nodeObj = nodeObj;
        this.nodeType = this._getNodeType(nodeObj);
        this.nodeName = this._getNodeName(nodeObj);
        this._textContent = null;
        this._childNodes = null;
    }

    _getNodeType(nodeObj) {
        if (nodeObj["#children"] || nodeObj["@"]) return domElmType.ELEMENT_NODE;
        if (nodeObj["#text"] !== undefined) return domElmType.TEXT_NODE;
        return 0;
    }

    _getNodeName(nodeObj) {
        return nodeObj["#name"] || "#document-fragment";
    }

    get textContent() {
        if (this._textContent !== null) return this._textContent;
        
        if (this.nodeType === domElmType.ELEMENT_NODE) {
            // 递归合并所有子节点文本
            let content = "";
            const children = this.childNodes;
            for (let i = 0; i < children.length; i++) {
                content += children.item(i).textContent;
            }
            this._textContent = content;
        } else {
            this._textContent = this.nodeObj["#text"] || "";
        }
    ...
    _createChildNodes() {
        ...
        const children = this.nodeObj["#children"] || [];
        const wrappedChildren = children.map(child => new FromXmlNodeWrapper(child));
        
        return {
            length: wrappedChildren.length,
            item: (index) => wrappedChildren[index]
        };
    }

    getAttribute(name) {
        if (this.nodeType !== domElmType.ELEMENT_NODE) return undefined;
        return this.nodeObj["@"]?.[name];  // 通过"@"访问属性
    }
    ...
    hasAttributes() {
        return !!this.nodeObj["@"] && Object.keys(this.nodeObj["@"]).length > 0;
    }
}
/**
 * Create a random ID.
