...
function zc_insertXMLNode(range, node, disableHyperlinks) {
    ...
    if (!node || node.nodeType === undefined) {
        console.warn(`${node} is not a valid node!`);
        return -1;
    }
            ...
            switch (_node.nodeName) {
                ...
                case 'SPAN':
                    ...
                    if (style.includes('text-decoration: line-through')) {
                        ...
                        const color = parseColor(style.split('color:')[1].split(';')[0], true);
                        if (color) _range.Font.TextColor.RGB = color;
                    } else if (style.includes('background-color')) {
                        const color = parseColor(style.split('background-color:')[1].split(';')[0], true);
                        if (color) _range.HighlightColorIndex = zc_matchHighlightColor(color);
                    }
                    break;
                case 'H1':
                ...
                case 'OL':
                    ...
                    break;
                case 'BLOCKQUOTE':
                    _range.Paragraphs.Format.LeftIndent = cm2pt(0.5);
                    break;
                case 'SUP':
                    _range.Font.Superscript = true;
                    break;
                case 'SUB':
                    _range.Font.Subscript = true;
                    break;
                case 'B':
                ...
                case 'U':
                    ...
                    break;
                default:
                    if (!common_tags.includes(_node.nodeName)) {
                        console.warn(`<${_node.nodeName}> formatting not implemented`);
                    }
                    break;
            }
        }
...
function zc_insertRichText(range, rich, disableHyperlinks) {
    ...
    // 确保XML有根元素[5](@ref)
    if (!/^<[a-z]/.test(xmlStr.trim())) {
        xmlStr = `<root>${xmlStr}</root>`;
    }

    const xml = parseXML(xmlStr);
    if (xml) {
        // 使用documentElement替代firstChild
        return zc_insertXMLNode(range, xml.childNodes[0], disableHyperlinks) >= 0;
    }
    console.warn(`XML解析失败: ${rich}`);
    return false;
}
/**
...
function zc_insertBibEntries(range, xmlStr, bibStyle) {
    ...
    const findNodesByClass = (node, className) => {
        const results = [];
        ...
        if (node.childNodes) {
            for (const child of node.childNodes) {
                results.push(...findNodesByClass(child, className));
            }
        }
        return results;
    };
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        const traverseChildNodes = (node) => {
            if (node.attributes?.class?.includes('csl-left-margin')) {
                left = node;
            } else if (node.attributes?.class?.includes('csl-right-inline')) {
                right = node;
            }
            if (node.childNodes) {
        ...
        if (left) {
            ...
            range.InsertAfter('\t'.repeat(bibStyle.tabStopsCount));  // 插入制表符
            range.Collapse(wdCollapseEnd);
            if (right) {
                zc_insertXMLNode(range, right, true);  // 插入右侧内容
                range.Collapse(wdCollapseEnd);
            }
        } else if (right) {
...
function createAndInsertHtml(range,str){
